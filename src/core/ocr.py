            ...
            for i, (x1, y1, x2, y2) in enumerate(bubble_coords):
                try:
                    ...
                    try:
                        ...
                        bubble_img_pil.save(os.path.join(debug_dir, f"bubble_{i}_{source_language}_baidu.webp"), format="WEBP",lossless=True)
        ...
        if ocr_instance:
            ...
            for i, (x1, y1, x2, y2) in enumerate(bubble_coords):
                try:
                    ...
                    try:
                        debug_dir = get_debug_dir("ocr_bubbles")
                        bubble_img_pil.save(os.path.join(debug_dir, f"bubble_{i}_{source_language}.webp"), format="WEBP",lossless=True)
            ...
            try:
                for i, (x1, y1, x2, y2) in enumerate(bubble_coords):
                    try:
                        ...
                        try:
                            debug_dir = get_debug_dir("ocr_bubbles")
                            bubble_img_pil.save(os.path.join(debug_dir, f"bubble_{i}_{source_language}_ai_vision.webp"), format="WEBP",lossless=True)
                        except Exception as save_e:
