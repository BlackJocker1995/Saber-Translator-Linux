...
def translate_image():
    ...
    try:
        ...
        if clean_image:
            ...
            clean_image.save(buffered_clean, format="WEBP",lossless=True)
            ...
            if clean_background:
                buffered_clean = io.BytesIO()
                clean_background.save(buffered_clean, format="WEBP",lossless=True)
                clean_img_str = base64.b64encode(buffered_clean.getvalue()).decode('utf-8')
        ...
        buffered = io.BytesIO()
        translated_image.save(buffered, format="WEBP",lossless=True)
...
def re_render_image():
    try:
        ...
        # 转换结果图像为Base64字符串
        buffered = io.BytesIO()
        rendered_image.save(buffered, format="WEBP",lossless=True)
        img_str = base64.b64encode(buffered.getvalue()).decode('utf-8')
